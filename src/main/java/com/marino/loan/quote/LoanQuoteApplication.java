...
public final class LoanQuoteApplication {
    ...
    final static String LENDERS_PATH = "src\\main\\resources\\market.csv";
     ...
     * @return
     */
    public static LoanQuote getLoanQuote(final String loanQuote){

        final LoanQuoteCalculator loanQuoteCalculator;
        final int loanAmount;

        try {
            loanQuoteCalculator = createLoanQuoteCalculator(LENDERS_PATH);

            loanAmount = getLoanAmount(loanQuote);
            ...
            throw new ResponseStatusException(
                    HttpStatus.BAD_REQUEST, "Insufficient offers from lenders to satisfy the loan. Try a smaller loan amount.");
        }
    }
}
