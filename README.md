...
```bash
$ git clone https://github.com/gianda/loan-quote-web.git
$ cd loan-quote-web
$ ./mvnw spring-boot:run
```

The Loan Quote Calculator can then be found at `http://localhost:8080/`.

## Usage

Just insert loan amount into the web form and check the best loan quote.

## Security

Input text is formatted to prevent SQL Injection.

## API

The Calculator simply responds to an HTTP GET where X is the loan amount:
`/loanquote?loanamount=X`

The response from the server is a JSON Object that can be later formatted by the frontend.

## Future Implementations

1. Of course remove CSV from server and reorganize lenders into a DB
0. Deeply check quote calculation methods and data structures to speed up performance
0. Add a new field in the form so the user can change loan duration for the quote.

## How is the quote calculated?

1. The list of lenders is read from the CSV file in `src\main\resources`
